        ...
        for (let i = 4; i < parsed[1].length; i++) {
            ...
            weeksData.push({
                ...
                admin: {
                    ...
                    leads: { target: cleanNumber(parsed[125]?.[i]), actual: cleanNumber(parsed[126]?.[i]), gfs: { line: cleanNumber(parsed[129]?.[i]), tel: cleanNumber(parsed[130]?.[i]), fb: cleanNumber(parsed[131]?.[i]) }, mhl: { line: cleanNumber(parsed[134]?.[i]), tel: cleanNumber(parsed[135]?.[i]), fb: cleanNumber(parsed[136]?.[i]) }, car: mappedNumber(parsed, buildingSourceRows.car, 'installsTotal', i, 406) },
